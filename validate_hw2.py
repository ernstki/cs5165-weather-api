...
def score_hw2( base_url):
    ...
    def validate_get(obj):
        print("validating GET: %s" % obj.url)
        score=0
        try:
    ...
    def validate_get_obj(obj,date):
        ...
        try:
            ...
            if ((len(data)==3) and ('TMAX' in data  ) and ('TMIN' in data)) :
                ...
                score=score+10
            if data['DATE']==date:
                print("L.2 Checking for DATE %s  OK" % date)
                score=score+10
    ...
    def validate_get_tmax(obj,tmax):
        ...
        try:
            ...
            if int(data['TMAX'])==tmax:
                print("G.3 Checking for TMAX %s   OK" % tmax)
                score=score+10
    ...
    def validate_post(obj,date):
        print("validating POST: %s" % obj.url)
        score=0
        try:
            ...
            if data['DATE']==date:
                print("P.2 Checking for DATE %s  OK" % date)
                score=score+10
    ...
    def validate_del(obj):
        print("validating DELETE: %s" % obj.url)
        score=0
        try:
    ...
    def validate_forcasts(obj,date):
        ...
        try:
            ...
            if len(data)==7:
                ...
                score=score+5
            if data[0]['DATE']==date:
                print("F.3 Checking Date %s  OK" % date)
                score=score+5

            if (float(data[6]['TMAX']) > -30.0) and (float(data[6]['TMAX']) < 150.0 ):
                print("F.4 Checking TMAX in range -30 and 100  " +
                      str(data[6]['TMAX']) + " for date " + date + " OK")
                score=score+5
    ...
    print("Total Score without BONUS Evaluation: %0.2f" % (100*total_score/130.0))
    ...
    print("Total Score after BONUS Evaluation: %0.2f" % total_score)
